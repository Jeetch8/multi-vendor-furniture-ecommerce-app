...
import { findValidCoupons, roundToTwoDecimals } from '@/utils/helpers';
...
function TotalPrice() {
  ...
  const handleCoupon = () => {
    const verifiedCoupons = cart?.cartItems
              ...
              {validCoupons.map(
                (coupon) =>
                  coupon.productCoupon && (
                    <div
                      key={coupon.productCoupon?.id}
                      className="flex justify-between"
                    >
                      <p className="text-xs">
                        Coupon Code: {coupon.productCoupon?.code}
                      </p>
                      <p className="text-xs font-semibold text-orange-1">
                        - ${coupon.productCoupon?.discountAmount}
                      </p>
                    </div>
                  )
              )}
