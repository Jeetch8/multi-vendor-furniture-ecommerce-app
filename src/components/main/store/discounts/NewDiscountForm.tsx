...
import { TProductToCategoryWithCategory } from '@/types/Category';

type NewDiscountFormProps = {
  products: TProductForTable[];
};
  ...
  const selectItemsCategories = Array.from(
    new Set(
      products.flatMap((product) =>
        product.categoryToProducts.map(
          (cat: TProductToCategoryWithCategory) => cat.category.id
        )
      )
    )
  )
    .map((id) => {
      const category = products.flatMap((product) =>
        product.categoryToProducts.find(
          (cat: TProductToCategoryWithCategory) => cat.category.id === id
        )
      );
  ...
  return (
    <Form action={action}>
      ...
      <Input
        ...
        error={formState.errors?.discountPercent}
      />
      ...
      <div className="flex flex-col gap-2">
        ...
        {formState.errors?.date && (
          <FormError>{formState.errors?.date}</FormError>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {formState.errors?.categoryId && (
          <FormError>{formState.errors?.categoryId}</FormError>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        {formState.errors?.productId && (
          <FormError>{formState.errors?.productId}</FormError>
        )}
      </div>
